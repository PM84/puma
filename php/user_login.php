...
if(isset($_POST['PostFktn'])){
	...
	include($_SERVER['DOCUMENT_ROOT']."/php/system.php");
...
function validateUsername($username,$uID){
	...
	$ergebnis=mysqli_query($verbindung,$query);
	if(mysqli_num_rows($ergebnis)>0){return false;}else{return true;}
}
...
function addUser($name,$vname,$geschlecht,$email,$username,$password,$bundesland,$SchulNr,$Edit_uID){
	...
	if(isset($_SESSION['t'])){$token=$_SESSION['t'];}else{$token="";} // DIESE Funktion wird nur für die Dokumentation der Studie benötigt. Kann später gelöscht werden!
