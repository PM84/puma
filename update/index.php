...
<html>
	...
	<body>
		...
		<div class="container">
			...
			$getVersions = file_get_contents('http://www.jdev.pemasoft.de/zCMS-UPDATE-PACKAGES/current-release-versions.txt');
			if ($getVersions != '')
			{
				$found="";
				$updated=false;
				...
				foreach ($versionList as $aV)
				{
					if ( $aV > get_siteInfo('CMS-Version')) {
						...
						if ( !is_file(  $_SERVER['DOCUMENT_ROOT'].$_SESSION['DOCUMENT_ROOT_DIR'].'/zUPDATES/MTv4-update-'.$aV.'.zip' )) {
							...
							if ( !is_dir( $_SERVER['DOCUMENT_ROOT'].$_SESSION['DOCUMENT_ROOT_DIR'].'/zUPDATES/' ) ) mkdir ( $_SERVER['DOCUMENT_ROOT'].$_SESSION['DOCUMENT_ROOT_DIR'].'/zUPDATES/' );
							$dlHandler = fopen($_SERVER['DOCUMENT_ROOT'].$_SESSION['DOCUMENT_ROOT_DIR'].'/zUPDATES/MTv4-update-'.$aV.'.zip', 'w');
						...
						if (isset($_GET['doUpdate']) && $_GET['doUpdate'] == true) {
							...
							$filename = $_SERVER['DOCUMENT_ROOT'].$_SESSION['DOCUMENT_ROOT_DIR'].'/zUPDATES/MTv4-update-'.$aV.'.zip';
							...
							include($_SERVER['DOCUMENT_ROOT'].$_SESSION['DOCUMENT_ROOT_DIR']."/update/update.php");
							$updated = TRUE;
						}
