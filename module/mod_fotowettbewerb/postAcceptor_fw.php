...
$imageFolder = $_SERVER['DOCUMENT_ROOT'].$_SESSION['DOCUMENT_ROOT_DIR']."/media/uploads/fotowettbewerb/";
// echo $imageFolder;
...
if (is_uploaded_file($temp['tmp_name'])){
	...
	// Verify extension
	if (!in_array(strtolower(pathinfo($temp['name'], PATHINFO_EXTENSION)), array("jpg", "png", "gif"))) {
...
/* 	// 	if ($_FILES['userfile']['size']>50*1024*1024*1024 && in_array($extension,array("mp4","mov","mv4","avi"))) {
	...
	*/
	...
	if (filesize($temp['tmp_name'])>1*1024*1024*1024 && in_array($extension,array("jpg","png","gif"))) {
		header("HTTP/1.0 502 Die Bilddatei ist gößer als 2MB.");
		return;
	}
	$titel=$temp['name'];
	$uniqid=uniqid();
	// Accept upload if there was no origin, or if it is an accepted origin
	$new_file_name=$kursID."_".$token."_".$uniqid.".".$extension;
	...
	echo json_encode(array('location' => "/media/uploads/fotowettbewerb/$new_file_name"));
	// 	  exit;
