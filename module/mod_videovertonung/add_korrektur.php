...
if (isset($_POST['beschreibung'])) {
	...
	$zu_fID = intval($_SESSION['edit_fID']);
	if (isset($_SESSION['kID'])) {
		...
		<div class="row" style=''>
			...
			<div class="col-md-10">
				<div class="row" style='margin-top:10px; text-align:center; background-color:lightgray; padding:10px; border-radius: 10px;'>
					...
					<div class="col-md-10">
						<p class="lead" style='margin:0'>Korrekturaufgabe zur Videovertonung hinzufügen</p>
					</div>
					<div class="col-md-1"></div>
				</div>
			</div>
		...
		<div class="row" style=''>
			...
			<div class="col-md-6">
				...
				<form action='' method="POST" style='margin:0;'>
					<input type="hidden" value='<?php if (isset($_SESSION['edit_fID'])) {
													echo intval($_SESSION['edit_fID']);
												} ?>' name='fID'>
					<input type="hidden" value='<?php if (isset($_SESSION['kID'])) {
													echo intval($_SESSION['kID']);
												} ?>' name='kID'>
					<p class="lead" style='margin-top:25px'>Allgemein</p>
					<div class="form-group">
						<label for="titel">Titel</label>
						<input id='titel' type="text" class="form-control" name="titel" placeholder="Titel der Aufgabe" required value='<?php if (isset($AufgabeInfo->titel)) {
																																			echo html_entity_decode($AufgabeInfo->titel, ENT_QUOTES, "UTF-8");
																																		} ?>'>
					</div>
					<div class="form-group">
						<label for="beschreibung">Beschreiben Sie die Aufgabe</label>
						<textarea class="form-control" id="beschreibung" name='beschreibung' rows="3"><?php if (isset($AufgabeInfo->beschreibung)) {
																											echo $AufgabeInfo->beschreibung;
																										} ?></textarea>
					</div>
					...
					<div class="form-group">
						...
						<select class="form-control" name="fGroups[]" id="fGroups" multiple>
							...
							?>
								<option value="<?php echo $fGruppe['FGroupID']; ?>" <?php if (isset($AufgabeInfo)) {
																							if (is_array($AufgabeInfo->fGroupsArr)) {
																								if (in_array($fGruppe['FGroupID'], $AufgabeInfo->fGroupsArr)) {
																									echo "selected";
																								}
																							}
																						} ?>><?php echo html_entity_decode($fGruppeParameter->GroupTitel, ENT_QUOTES, "UTF-8"); ?></option>
							<?php
							}
							?>
						</select>
						...
						<select class="form-control" name="fIDs[]" id="fIDs" multiple>
							...
							?>
								<option value="<?php echo $frage['FrageID']; ?>" <?php if (isset($AufgabeInfo)) {
																						if (is_array($AufgabeInfo->fArr)) {
																							if (in_array($frage['FrageID'], $AufgabeInfo->fArr)) {
																								echo "selected";
																							}
																						}
																					} ?>><?php echo html_entity_decode($frageParameter->titel, ENT_QUOTES, "UTF-8"); ?></option>
							<?php
							}
							?>
						</select>
...
//-->
					...
					<input type="submit" class="btn btn-primary" style='' value="<?php if (isset($_SESSION['kID'])) {
																						echo "Aufgabe updaten";
																					} else {
																						echo "Aufgabe eintragen";
																					}  ?>">
				</form>
			</div>
