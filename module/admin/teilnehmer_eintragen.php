...
if (isset($_FILES["file"])) {
	...
	if ($_FILES["file"]["error"] > 0) {
		...
		if (file_exists($_SERVER['DOCUMENT_ROOT'] . $_SESSION['DOCUMENT_ROOT_DIR'] . "/tmp/" . $_FILES["file"]["name"])) {
			echo $_FILES["file"]["name"] . " already exists. ";
		} else {
			$storagename = $uID . "_" . uniqid() . ".txt";
			move_uploaded_file($_FILES["file"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . $_SESSION['DOCUMENT_ROOT_DIR'] . "/tmp/" . $storagename);
		}
	}

	if ($file = fopen($_SERVER['DOCUMENT_ROOT'] . $_SESSION['DOCUMENT_ROOT_DIR'] . "/tmp/" . $storagename, 'r')) {
				...
				?>
					...
					<form class='' method="POST" action="">
						<div class="form-group">
							...
							<select class="form-control" id="geschl" name="geschlecht" required>
								<option value='m' <?php if (isset($TeilnehmerInfo['geschlecht'])) {
														if ($TeilnehmerInfo['geschlecht'] == "m") {
															echo "selected";
														}
													} ?>>männlich</option>
								<option value='w' <?php if (isset($TeilnehmerInfo['geschlecht'])) {
														if ($TeilnehmerInfo['geschlecht'] == "w") {
															echo "selected";
														}
													} ?>>weiblich</option>
							</select>
						...
						<div class="form-group">
							...
							<input id='vname' type="text" class="form-control" name="vname" placeholder="Vorname" required value='<?php if (isset($TeilnehmerInfo['vname'])) {
																																		echo $TeilnehmerInfo['vname'];
																																	} ?>'>
						</div>
						<div class="form-group">
							<label for="name">Name</label>
							<input id='name' type="text" class="form-control" name="name" placeholder="Name" required value='<?php if (isset($TeilnehmerInfo['name'])) {
																																	echo $TeilnehmerInfo['name'];
																																} ?>'>
						</div>
						<div class="form-group">
							<label for="eMail">eMail</label>
							<input id='eMail' type="text" class="form-control" name="email" placeholder="eMail" required value='<?php if (isset($TeilnehmerInfo['email'])) {
																																	echo $TeilnehmerInfo['email'];
																																} ?>'>
						</div>
					...
					<form class='' method="POST" action="" enctype="multipart/form-data">
						<div class="form-group">
							...
							<small id="fileHelp" class="form-text text-muted"><code>Die Import-Datei muss die Struktur dieser <a target="_blank" href='<?php echo $_SESSION['DOCUMENT_ROOT_DIR']; ?>/vorlagen/Vorlage_TeilnehmerImport.txt'>Demo-Datei</a> aufweisen. Die erste Zeile enthält die Überschriften und darf keinesfalls gelöscht oder verändert werden!</code></small>
						</div>
			...
			<div class="col-md-4">
				...
				?>
					<p class="lead">Kursteilnehmer
						<a href='<?php echo $_SESSION['DOCUMENT_ROOT_DIR']; ?>/module/admin/token_drucken.php' class='glyphicon glyphicon-print' target="blank"></a>
					...
					<div class="row">
						<table class='table-striped' style='width:100%'>
							<tr>
								<th>Vorname</th>
								<th>Name</th>
								<th class="hidden-sm hidden-xs">Token</th>
								<th>Optionen</th>
							</tr>
							<?php
								...
								<tr>
									...
									<div class="row" style=''>
										<div class="col-sm-6 col-xs-6">
											<div>
												<?php if ($tn['dozent'] == 0) { ?>
												<img src="<?php echo $_SESSION['DOCUMENT_ROOT_DIR']; ?>/images/professor_red.svg" style="height:22px;">
												<?php } else { ?>
												<img src="<?php echo $_SESSION['DOCUMENT_ROOT_DIR']; ?>/images/professor_green.svg" style="height:22px;">
												<?php	   } ?>
											</div>
										</div>
										<div class="col-sm-6 col-xs-6">
									...
									<td class="" style='text-align:right;'>
										<div class="row" style=''>
											...
											<div class="col-md-2">
												<div style="padding: 5px 0;"><span class="tn_icon_used <?php if (count($abArr) > 0) {
																											echo 'tn_not_used';
																										} else {
																											echo 'tn_used';
																										} ?>" <?php if (count($abArr) > 0) {
																																													echo 'data-toggle="tooltip" title="Dieser Zugriffstoken wurde bereits verwendet"';
																																												} else {
																																													echo 'data-toggle="tooltip" title="Dieser Zugriffstoken wurde noch nicht verwendet"';
																																												} ?>></span></div>
											</div>
											<div class="col-md-5">
												<form action='' method="POST" style='display: inline-block; margin:0; width:100%;'>
