...
if($uID==0){
	header("LOCATION: /module/user/logout.php");
}
	...
	<body>
			...
			<div class="row" style='margin-top:50px;'>
				...
				<div class="col-md-5">
					...
					<form class=''  method="POST" action="">
						<input type="hidden" value="<?php if(isset($_SESSION['k_edit'])){ echo $_SESSION['k_edit'];} ?>" name="k">
						<div class="form-group">
							<label for="KT">Bitte geben Sie den Titel des Kurses ein</label>
							<input id='KT' type="text" class="form-control" name="kurstitel" placeholder="Kurstitel" required value='<?php if(isset($KursInfo['titel'])){echo $KursInfo['titel'];} ?>'>
						</div>

						<div class="form-group">
							<label for="KB">Beschreiben Sie den Kurs</label>
							<textarea id="KB" class="form-control" name="beschreibung" rows="3"><?php if(isset($KursInfo['beschreibung'])){ echo $KursInfo['beschreibung'];} ?></textarea>
						</div>
						...
						<div class="form-group">
							...
							<select class="form-control" id="kTypID" name="kTyp" required>
								...
								?>
								<option value=1 <?php if(isset($KursInfo['kTyp'])){ if($KursInfo['kTyp']==1){echo "selected";}} ?>>Einzelaufgaben (auch mehrere)</option>
								<?php } ?>

								<option value=2 <?php if(isset($KursInfo['kTyp'])){if($KursInfo['kTyp']==2){echo "selected";}} ?>>Mehrere zusammenhängende/aufeinander aufbauende Aufgaben</option>
							</select>
						</div>
				...
				<div class="col-md-5">
					...
					<div class="row" style="margin-bottom:3px;">
						<div class="col-md-11"  style='padding-right:0; padding-left:10px;'><form action='' method="POST" style='margin:0;'>
							<input type=hidden value='<?php  echo $KursInfo['kursID']; ?>' name='k'>
							<input type="submit" class="btn <?php if(isset($_SESSION['k_edit']) && $_SESSION['k_edit']==$KursInfo['kursID']){echo "btn-primary";}else{echo "btn-default";}?>" style='width:100%' value="<?php echo $KursInfo['titel']; ?>">
							</form>
						</div>
		...
		<script>
			...
			$(".embed_btn").on("click", function () {
				...
				$.ajax({
					type: "POST",
					data:{PostFktn: "get_Embed_Link", kursID:KursID},
					dataType: 'text',
